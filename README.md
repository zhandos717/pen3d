...
`examples/` holds ready assemblies — open them with **Открыть** in the editor:

| file | what it is |
|---|---|
| `korpus-platy` | 80×60×25 enclosure: floor, four walls, screw bosses, connector cutout |
| `kronshtein-L` | L-bracket with a gusset and six mounting holes |
| `khomut-25` | split clamp for 25 mm pipe with bolt ears |
| `podstavka-telefon` | phone stand with a cable slot |
| `shkiv-608` | pulley with a seat for a 608 bearing |

Every one is generated by `examples/build.py` and passes `check_scene` — the same printability validator the AI agent is held to. Regenerate them with `.venv/bin/python examples/build.py --write`.

Note the CSG order when building your own: all solids are unioned first, holes are subtracted after. A cavity carved out of a box will therefore also eat any boss standing inside it — build enclosures from a floor plus walls instead.
